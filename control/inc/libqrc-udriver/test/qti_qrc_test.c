...
int main(int argc, char** argv)
{
	...
	while ((ret = getopt_long(argc, argv, "d:g:r:", long_options, &option_index)) != -1) {
		switch (ret) {
			case 'd':
				dev = optarg;
				break;
	...
	initialized = true;
	char rx_buffer[MAX_DATA_LEN];
	ssize_t num = -1;
	char tx_buffer[MAX_DATA_LEN];
	size_t length = MAX_DATA_LEN;
	char str[10] = {0};
	char option;
	size_t sum;
	int count;
